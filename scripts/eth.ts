...
async function main() {
    ...
    await vault.connect(signers[1])["withdraw(uint256,address,address,uint256)"](assets1, signers[1].address, signers[1].address, 0);
    ...
    await vault.connect(signers[3])["redeem(uint256,address,address,uint256)"](shares3, signers[3].address, signers[3].address, 0);
